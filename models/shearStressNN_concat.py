...
class ShearStressNN(nn.Module):
    def __init__(self, branch_dims=[25, 64, 64, 64], trunk_dims=[3, 64, 64, 64],
                 combined_dims=[128, 128, 64], dropout=0.1):
        """
        Neural network for predicting shear stress using spherical harmonic coefficients
        Uses concatenation instead of multiplication to combine branch and trunk networks
        ...
        Args:
            ...
            combined_dims: Dimensions for the combined network after concatenation
        ...
        # Input size is branch_dims[-1] + trunk_dims[-1]
        modules = []
        in_channels = branch_dims[-1] + trunk_dims[-1]
        for h_dim in combined_dims:
            modules.append(nn.Sequential(
                nn.Linear(in_channels, h_dim),
                nn.LayerNorm(h_dim),
                nn.Tanh(),
            ...
            in_channels = h_dim
        self._combined = nn.Sequential(*modules)

        # Output layer
        self._out_layer = nn.Linear(combined_dims[-1], 3)
    ...
    def forward(self, coeffs, xyz):
        ...
        # Concatenate branch and trunk outputs
        y_combined = torch.cat([y_br, y_tr], dim=1)

        # Pass through combined network
        y_combined = self._combined(y_combined)
    ...
    def loss(self, coeffs, xyz, values, reg_lambda=1):
        ...
        l2_loss = torch.sqrt(torch.mean(torch.sum((y_pred - values) ** 2, dim=1), dim=0))
        return l2_loss
